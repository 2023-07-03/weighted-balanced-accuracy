...
class TextCNN(object): 
    def __init__(self, char_ngram_vocab_size, word_ngram_vocab_size, char_vocab_size, \
        ...
        filter_sizes=[3,4,5,6], mode=0, apply_weights=False, class_count=2):
        if mode == 4 or mode == 5: 
        ...
        self.input_y = tf.placeholder(tf.float32, [None, class_count], name="input_y")
        self.input_weights = tf.placeholder(tf.float32, [None, 1], name="input_weights")
        ...
        with tf.name_scope("output"): 
            ...
            w = tf.get_variable("w", shape=(128,self.class_count), initializer=tf.contrib.layers.xavier_initializer())
            b = tf.Variable(tf.constant(0.1, shape=[self.class_count]), name="b")
        ...
        with tf.name_scope("loss"): 
            ...
            print('printing_losses shape', tf.shape( losses, out_type=tf.dtypes.int32, name=None ))
            # weights=
            if self.apply_weights:
                self.loss = tf.reduce_mean(losses * tf.squeeze(self.input_weights)) + l2_reg_lambda * l2_loss
            else:
