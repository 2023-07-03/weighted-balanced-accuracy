...
parser.add_argument('--model.emb_mode', type=int, default=default_emb_mode, metavar="EMBMODE",
  help="1: charCNN, 2: wordCNN, 3: char + wordCNN, 4: char-level wordCNN, 5: char + "
       "char-level wordCNN (default: {})".format(default_emb_mode))
default_apply_weights = False
parser.add_argument('--model.apply_weights', type=bool, default=default_apply_weights, metavar='APPLYWEIGHTS',
  help="boolean variable, whether to apply weights (default: {})".format(default_apply_weights))
default_preset_weights = ''
parser.add_argument('--model.preset_weights', type=str, default=default_preset_weights, metavar='PRESETWEIGHTS',
  help="string variable, weight values connected by comma, to set weights per lable, e.g., '0.1,0.3,0.2,0.4' "
       "means per-label-weights are '1:0.1,2:0.3,3:0.2,4:0.4'. If not set, do not use preset weights")

# train args 
default_nb_epochs = 50
...
for i in range(len(labels)):
    label = int(labels[i])
    if label not in label_indices_mapping:
        label_indices_mapping[label] = []
    label_indices_mapping[label].append(i)
print("Overall data stats: ")
for lb, indices in label_indices_mapping.items():
    print('==label', lb, 'count', len(indices))


x_train, y_train, sample_weights_train, x_test, y_test, sample_weights_test = \
    prep_train_test(label_indices_mapping, FLAGS["data.dev_pct"], preset_weights=FLAGS["model.preset_weights"])
...
def train_dev_step(x, y, sample_weights, emb_mode, is_train=True):
    if is_train: 
        p = 0.5
    else: 
        p = 1.0
    if emb_mode == 1: 
        feed_dict = {
            ...
            cnn.input_y: y,
            cnn.input_weights: sample_weights,
...
def make_batches(x_train_char_seq, x_train_word, x_train_char, y_train, sample_weights_train, batch_size, nb_epochs, shuffle=False):
    if FLAGS["model.emb_mode"] == 1:  
        batch_data = list(zip(x_train_char_seq, y_train, sample_weights_train))
    elif FLAGS["model.emb_mode"] == 2:  
        batch_data = list(zip(x_train_word, y_train, sample_weights_train))
    elif FLAGS["model.emb_mode"] == 3:  
        batch_data = list(zip(x_train_char_seq, x_train_word, y_train, sample_weights_train))
    elif FLAGS["model.emb_mode"] == 4:
         batch_data = list(zip(x_train_char, x_train_word, y_train, sample_weights_train))
    elif FLAGS["model.emb_mode"] == 5:  
        batch_data = list(zip(x_train_char, x_train_word, x_train_char_seq, y_train, sample_weights_train))
...
def prep_batches(batch):
    if FLAGS["model.emb_mode"] == 1:
        x_char_seq, y_batch, sample_weights_train = zip(*batch)
    elif FLAGS["model.emb_mode"] == 2:
        x_word, y_batch, sample_weights_train = zip(*batch)
    elif FLAGS["model.emb_mode"] == 3:
        x_char_seq, x_word, y_batch, sample_weights_train = zip(*batch)
    elif FLAGS["model.emb_mode"] == 4:
        x_char, x_word, y_batch, sample_weights_train = zip(*batch)
    elif FLAGS["model.emb_mode"] == 5:
        x_char, x_word, x_char_seq, y_batch, sample_weights_train = zip(*batch)
    ...
    return x_batch, y_batch, sample_weights_train
...
with tf.Graph().as_default(): 
    ...
    with sess.as_default():  
        cnn = TextCNN(
                ...
                apply_weights = FLAGS["model.apply_weights"],
                class_count = len(label_indices_mapping))

        print('apply_weights', FLAGS["model.apply_weights"])
        ...
        train_batches, nb_batches_per_epoch, nb_batches = make_batches(x_train_char_seq, x_train_word, x_train_char, y_train,
                                                                       sample_weights_train, FLAGS["train.batch_size"], FLAGS['train.nb_epochs'], True)
        ...
        for idx in it:
            ...
            x_batch, y_batch, sample_weights_train_batch = prep_batches(batch)
            step, loss, acc = train_dev_step(x_batch, y_batch, sample_weights_train_batch, emb_mode=FLAGS["model.emb_mode"], is_train=True)
            ...
            if step % FLAGS["log.eval_every"] == 0 or idx == (nb_batches-1): 
                ...
                nb_corrects = 0
                nb_instances = 0
                test_batches =  make_batches(x_test_char_seq, x_test_word, x_test_char, y_test, sample_weights_test, FLAGS['train.batch_size'], 1, False)
                for test_batch in test_batches:
                    x_test_batch, y_test_batch, sample_weights_test_batch = prep_batches(test_batch)
                    step, batch_dev_loss, batch_dev_acc = train_dev_step(x_test_batch, y_test_batch, sample_weights_test_batch, emb_mode=FLAGS["model.emb_mode"], is_train=False)
